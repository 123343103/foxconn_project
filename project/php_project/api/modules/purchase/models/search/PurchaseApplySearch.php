 ...
 */
class PurchaseApplySearch extends SalePurchasenoteh
{
    ...
    {
        return [
            [['searchKeyword', 'notify_no', 'notfy_status', 'bill_type', 'notify_foganid', 'notify_from', 'saph_code', 'notify_toganid'], 'safe'],
        ];
    }
...
//            ->LeftJoin('erp.partno_price pp', "pp.part_no=pdt.pdt_no")
            ->leftJoin('bs_category_unit cu', 'cu.id=pdt.unit')
            ->LeftJoin('bs_brand bb', "bb.brand_id=pdt.brand_id")
            ->where('pdt.pdt_id>0 and ctg.category_id  is not null');
