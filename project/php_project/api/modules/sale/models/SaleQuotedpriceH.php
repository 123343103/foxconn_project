...
class SaleQuotedpriceH extends Common
{
    ...
    {
        return [
            [['origin_id', 'p_bill_id', 'comp_id', 'saph_type', 'corporate', 'trade_mode', 'title_district', 'send_district', 'cust_id', 'organization_id', 'sts_id', 'staff_id', 'DISTRICT_ID', 'cur_id', 'pat_id', 'pac_id', 'pay_type', 'dec_id', 'district_id2', 'sell_manager', 'sell_delegate', 'origin_hid', 'saph_reviewby', 'saph_flag', 'create_by', 'update_by','invoice_type'], 'integer'],
            [['saph_date', 'delivery_date', 'saph_limdate', 'rdate', 'cdate', 'udate'], 'safe'],
            [['quantity', 'bill_oamount', 'bill_camount', 'bill_freight'], 'number'],
            [['bus_code', 'saph_code', 'csarea_id', 'cust_contact', 'cust_tel', 'contract_no', 'logistics_type'], 'string', 'max' => 20],
    ...
    {
        return $this->hasMany(SaleQuotedpriceL::className(),['saph_id'=>'saph_id']);
    }
    /**
     * 关联客户
     */
