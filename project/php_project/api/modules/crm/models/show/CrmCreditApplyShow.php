...
class CrmCreditApplyShow extends LCrmCreditApply
{
...
//        };
        ...
        //营业额币别
        $fields['t_currency'] = function(){
            $a = $this->turnover;
            $b = BsPubdata::findOne($a[0]['currency']);
            return $b;
        };
        ...
        $fields['limit'] = function(){
            ...
            if(!empty($this->creditLimit)){
                foreach ($this->creditLimit as $kl=>$vl){
                    ...
                    $limit[$kl]['l_limit_id'] = $vl['l_limit_id'];
        ...
        $fields['file1'] = function(){
            ...
            if(!empty($this->creditFile1)){
                foreach ($this->creditFile1 as $key1=>$val1){
                    $file1[$key1]['date_file'] = substr($val1->file_new, 2, 6);
        ...
        $fields['file2'] = function(){
            ...
            if(!empty($this->creditFile2)){
                foreach ($this->creditFile2 as $key2=>$val2){
                    $file2[$key2]['date_file'] = substr($val2->file_new, 2, 6);
