 ...
 */
class OrdDelRptSearch extends OrdInfo
{
    public $csarea_name;     //营销区域
    public $ord_status;      //订单状态
    public $cust_department; //销售部门
    public $category_sname;  //商品分类
    public $tp_spec;         //商品规格
    ...
    {
        return [
            [['csarea_name', 'ord_no', 'ord_status', 'cust_department', 'tp_spec',  'cust_shortname', 'cust_code', 'staff_name', 'start_date', 'end_date',
               ], 'safe'],
    ...
    public function search($params)
    {
        $query = (new Query())
            ->select([
                ...
                'b.ord_no',
                // 付款方式
                '(CASE WHEN d2.bsp_svalue is not null THEN d2.bsp_svalue ELSE e.pac_sname END) as pac_sname',
                //商品类别
                ...
                . 'WHEN 0 THEN "无定价" '
                . 'WHEN -1 THEN "面议" '
                . 'ELSE FORMAT((r.price),5) END) as uprice_tax_o',
                //币别
                'd.bsp_svalue',
                //金额（含税）
                'FORMAT(a.tprice_tax_o,2) tprice_tax_o',
                //本币金额（含税）
                'FORMAT(a.tprice_tax_c,2) tprice_tax_c',
            ])
            ->from(['a' => 'oms.ord_dt'])
            ...
            ->leftJoin('pdt.bs_price r', 'a.prt_pkid=r.prt_pkid and b.cur_id=r.currency')//价格
            ->where((
                (
                    'ISNULL(r.maxqty)'
                    AND 'a.sapl_quantity >= r.minqty'
                )
                OR (
                    '! ISNULL(r.maxqty)'
                    AND 'a.sapl_quantity >= r.minqty'
                    AND 'a.sapl_quantity <= r.maxqty'
                )
            ))
            ->groupBy("a.ord_id")
        ...
        $query->andFilterWhere([
            ...
            'p.catg_id' => $this ->category_sname, //商品类别
            'i.organization_code' => $this->cust_department //销售部门
        ]);

        $trans = new Trans();
        $query->andFilterWhere(['like', 'b.ord_no', trim($this->ord_no)])// 订单编号
        ->andFilterWhere(['b.os_id' => $this->ord_status])// 订单状态
        ->andFilterWhere(['like', 'g.staff_code', trim($this->staff_name)])// 销售人员
        ->andFilterWhere(['like', 'n.tp_spec', trim($this->tp_spec)])// 商品规格型号
//        ->andFilterWhere(['like', 'h.cust_code', trim($this->cust_code)])// 客户代码
        ->andFilterWhere([
            'or',
