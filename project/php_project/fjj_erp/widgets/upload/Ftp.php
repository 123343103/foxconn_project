...
class Ftp {
    ...
    public function matchFiles($path, $allowFiles=[], &$files = array()){
        ...
        foreach ($items as $item){
            if($this->ftp_dir_exists(trim($path,"/")."/".trim($item,"/"))){
                $this->matchFiles((trim($path,"/")."/".trim($item,"/")),$allowFiles);
            }else{
                if(count($allowFiles)>0){
                    if(in_array(substr(strrchr($item,"."),1),$allowFiles)){
                        $data[]=[
                            "url"=>trim($this->config["httpIP"],"/")."/".trim($path,"/")."/".trim($item,"/")
                        ];
                    }
                }else{
                    $data[]=[
    ...
    public function ftp_file_exists($name){
        if(ftp_size($this->con,$name) > 0){
            return true;
        }else{
            return false;
        }
