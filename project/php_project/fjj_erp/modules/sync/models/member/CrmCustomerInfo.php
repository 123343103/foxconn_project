...
class CrmCustomerInfo extends \yii\db\ActiveRecord
{
...
//        return $this->hasOne(CrmEmployee::className(),['staff_code'=>'ccpich_personid'])->via('personinch');
//    }
//
...
//        return $this->hasOne(BsCategory::className(),['category_id'=>'member_reqitemclass']);
//    }
//    /*获取会员信息*/
//    public static function getMemberOne($id){
//        return self::find()->where(['cust_id'=>$id])->one();
//    }
    /**
     * @inheritdoc
    ...
    public function rules()
    {
        return [
//            [['cust_sname'],'unique'],
            [['staff_id', 'cust_area', 'cust_salearea', 'cust_district_1', 'cust_district_2', 'cust_district_3', 'custd_id', 'ccper_id', 'cpurch_id', 'personinch_status', 'cust_sales', 'cust_assistant', 'cust_verifyperson', 'cust_ismember', 'member_type', 'member_points', 'member_level', 'member_source', 'member_regweb', 'member_reqflag', 'member_visitflag', 'member_regcurr', 'member_curr', 'company_id', 'create_by', 'update_by'], 'integer'],
            [['cust_regdate', 'cust_veriftydate', 'member_regtime', 'member_certification', 'create_at', 'update_at'], 'safe'],
            [['member_compsum', 'cust_regfunds'], 'number'],
            [['cust_code', 'cust_level', 'cust_type', 'cust_class', 'cust_cource', 'cust_compvirtue', 'cust_inchargeperson', 'cust_tel1', 'cust_tel2', 'cust_fax', 'cust_businesstype', 'cust_compscale', 'cust_personqty', 'cust_filernumber', 'cust_industrytype', 'cust_pruchaseway', 'cust_bigseason', 'cust_pruchaseqty', 'cust_brand', 'pac_id', 'pat_id', 'cust_credittype', 'cust_creditqty', 'cust_isverify','member_name','member_compzipcode','member_reqitemclass','cust_position'], 'string', 'max' => 20],
            [['cust_sname', 'cust_eshortname','cust_email', 'cust_contacts', 'cmp_code'], 'string', 'max' => 100],
            [['cust_shortname'], 'string', 'max' => 60],
            [['cust_ename', 'member_reqdesription', 'member_businessarea', 'member_compwebside', 'member_marketing', 'member_compcust', 'member_compreq', 'member_remark','member_reqcharacter'], 'string', 'max' => 200],
            [['cust_flag', 'cust_status', 'cust_islisted' ,'three_to_one', 'com_type', 'invo_type', 'data_from'], 'string', 'max' => 2],
            [['cust_readress', 'cust_adress', 'cust_parentcomp', 'cust_belinkcomp', 'cust_maingoods'], 'string', 'max' => 120],
            [['cust_headquarters_address', 'cust_key_customer', 'cust_regname', 'cust_regnumber', 'cust_tax_code'], 'string', 'max' => 255],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'member_remark' => '会员备注',
            'company_id' => '所属厂商ID',
            'com_type' => '帳信類型 0:預付款會員;1:帳信會員',
            'invo_type' => '發票類型：0,不需要發票;1,增值稅普通發票;2,增值稅專用發票',
            'three_to_one' => '是否三証合一。0否，1是，2五証合一',
            'data_from' => '數據來源 2 表示富貿商城導入',
            'cmp_code' => '富貿商城導入的cmp_code',
            'create_by' => 'Create By',
            'create_at' => 'Create At',
            'update_by' => 'Update By',
            'update_at' => 'Update At',
...
//    {
//        $query = CrmCustomerInfo::find()->select('cust_shortname,cust_adress,cust_sname,cust_filernumber,cust_id,cust_code')->joinWith('status')->where(["!=", "sale_status", CrmCustomerStatus::STATUS_DEL])
