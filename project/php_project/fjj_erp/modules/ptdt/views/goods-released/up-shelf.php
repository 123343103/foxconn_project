...
<div class="content">
    ...
    <div>
        <div class="mb-10">
            ...
            <input type="hidden" name="catg_id" value="<?=isset($model['catg_id'])?$model['catg_id']:''?>">
            <span class="value-align"><?=isset($model["cat_three_level"])?$model["cat_three_level"]:'';?></span>
        </div>
        <div class="mb-10">
            <label class="label-width label-align">品名：</label>
            <input name="pdt_name" class="value-width value-align  add-require easyui-validatebox " readonly="true" data-options=""    type="text" value="<?=isset($model['pdt_name'])?$model['pdt_name']:''?>" maxlength="100">
            <label class="label-width label-align">品牌：</label>
            <?=Html::dropDownList("brand_id",isset($model["brand_id"])?$model["brand_id"]:'',$options["brands"],["prompt"=>"请选择","class"=>"value-width value-align easyui-validatebox"])?>
        </div>
        <div class="mb-10">
            <label class="label-width label-align" for="">商品标题：</label>
            <input class="value-width value-align easyui-validatebox" data-options="" type="text" name="pdt_title" value="<?=isset($model['pdt_title'])?$model['pdt_title']:''?>" maxlength="200">

            <label class="label-width label-align" for="">商品关键字：</label>
            <input class="value-width value-align" type="text" name="pdt_keyword" value="<?=isset($model['pdt_keyword'])?$model['pdt_keyword']:''?>" maxlength="200">
            <span class="red">多个关键词用“，”字符间隔</span>
        </div>
        <div class="mb-10">
            <label class="label-width label-align">商品标签：</label>
            <input class="value-width value-align easyui-validatebox" type="text" name="pdt_label" value="<?=isset($model['pdt_label'])?$model['pdt_label']:''?>" maxlength="100">
            <label class="label-width label-align"><span class="red">*</span>商品属性：</label>
            <?=Html::dropDownList("pdt_attribute",isset($model["pdt_attribute"])?$model["pdt_attribute"]:'',$options["pdt_attribute"],["prompt"=>"请选择","class"=>"value-width value-align easyui-validatebox","data-options"=>"required:true"])?>
        </div>
        <div class="mb-10">
            <label class="label-width label-align"><span class="red">*</span>商品形态：</label>
            <?=Html::dropDownList("pdt_form",isset($model["pdt_form"])?$model["pdt_form"]:'',$options["pdt_form"],["prompt"=>"请选择","class"=>"value-width value-align easyui-validatebox","data-options"=>"required:true"])?>
            <label class="label-width label-align"><span class="red">*</span>计量单位：</label>
            <?=Html::dropDownList("unit",isset($model["unit"])?$model["unit"]:'',$options["pdt_unit"],["prompt"=>"请选择","class"=>"value-width value-align easyui-validatebox","data-options"=>"required:true"])?>
        </div>
    </div>
    ...
    <div class="ml-10">
        <div class="mb-10">
            ...
            <div style="margin-left:140px;">
                <?=\app\widgets\upload\PreviewUploadWidget::widget([
                    "name"=>"pdt_img[]",
                    "extensions"=>"png,jpg",
                    "items"=>isset($model['pdt_img'])?$model['pdt_img']:[]
                ])?>
            </div>
        </div>
        <div class="mb-10">
        ...
        <div class="mb-10">
            ...
            <input style="position: relative;left:-5px;top:-1px;width:300px;text-indent: 1em;" type="text" id="upload3D" name="upload3D" value="<?=isset($model['upload3D'])?$model['upload3D']:''?>" readonly />
        </div>
        ...
        <div class="_ueditor">
            <?=Ueditor::widget([
                ...
                'id'=>'editor',
                'name'=>'details',
                'content'=>isset($model["details"])?$model["details"]:''
            ])?>
        </div>
    </div>
    ...
    <div class="mb-10">
        <table class="table">
            ...
            <?php if(isset($model["related_product"]) && count($model["related_product"])>0){ ?>
