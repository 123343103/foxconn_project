...
<script>
    $(function () {
        "use strict";
        $("#data").datagrid({
            url: "<?=Yii::$app->request->getHostInfo() . Yii::$app->request->url?>?id=" + <?= $id ?>,
            rownumbers: true,
            method: "get",
            idField: "type_id",
//          fitColumns:true,
            ...
            singleSelect: true,
            columns: [[
                {field: 'type_id', title: 'ID', width: 90},
                {field: 'type_no', title: '编码', width: 90},
                {field: 'type_name', title: '类名', width: 90},
                {field: 'type_title', title: '标题', width: 95},
                {field: 'type_keyword', title: '关键词', width: 100},
                {field: 'create_at', title: '创建时间', width: 130},
                {
                    field: "createBy", title: "创建人", width: 50, formatter: function (value, row, index) {
                    if (row.createBy) {
                ...
                {field: 'update_at', title: '修改时间', width: 130},
                {
                    field: "updateBy", title: "更新人", width: 50, formatter: function (value, row, index) {
                    if (row.updateBy) {
                ...
                layer.confirm("确定要删除这条记录吗",
                    ...
                    function () {
                        $.ajax({
                            ...
                            dataType: "json",
                            data: {"id": selectId},
                            url: "<?=Url::to(['/ptdt/product-type-maintain/delete']) ?>",
