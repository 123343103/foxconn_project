...
<div class="content">
    ...
    <div class="text-center">
        <button class="button-blue-big" id="pass">通过</button>
        <button class="button-blue-big ml-20" id="reject">驳回</button>
        <button class="button-white-big ml-20" onclick="window.location.href = '<?= Url::to(["index"])?>'">返回</button>
    </div>
</div>
...
<script>
    $(function () {
        ...
        $("#pass").on("click",function () {
            var id = <?= $applyModel->apply_id ?>;
            layer.confirm("确定这条记录审核通过吗?",
                {
                    btn:['确定', '取消'],
                    icon:2
                },
                function () {
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {"id": id},
                        url: "<?= Url::to(['firm-evaluate-apply/check-pass']) ?>",
            ...
            layer.confirm("确定要驳回这条记录吗?",
                ...
                function () {
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {"id": id},
                        url: "<?= Url::to(['firm-evaluate-apply/reject']) ?>",
                        success: function (data) {
                            if( data.flag === 1){
                                layer.alert(data.msg,{icon:1},function(){
                                    location.href = data.url;
