...
//                        });
                        ...
                        if($("#submitType").val()=="check"){
                            layer.confirm("确定要料号为："+$("#partno").text()+"的商品上架吗？",{icon:2},function () {
                                layer.closeAll();
        ...
        $("#partno-data").datagrid({
            ...
            columns: [[
                ...
                {field:'action',title:'操作',width:200,formatter:function(value,row,index){
                    ...
                    if("<?=$status?>"=="notupshelf"){//未上架
                        return "&nbsp;&nbsp;<i><a onclick='upshelf("+row.l_prt_pkid+")'>提交商品</a></i>&nbsp;&nbsp;<i><a href='javascript:void(0)' onclick='editPartno("+row.prt_pkid+")'>维护料号信息</a></i>&nbsp;&nbsp;";
                    }

                    if("<?=$status?>"=="selling"){//上架完成
                        if(row.part_status==4 && row.check_status!=2){
                            return "&nbsp;&nbsp;<i><a href='javascript:void(0)' style='color:gray'>下架商品</a></i>&nbsp;&nbsp;<i><a href='javascript:void(0)' onclick='checkProgress("+row.l_prt_pkid+")'>审核中</a></i>&nbsp;&nbsp;";
                        }else{
                            return "&nbsp;&nbsp;<i><a href='javascript:void(0)'  onclick=\"downshelf("+row.prt_pkid+",'"+row.part_no+"')\">下架商品</a></i>&nbsp;&nbsp;<i><a onclick='editPdt(event,"+row.prt_pkid+")'>编辑商品</a></i>&nbsp;&nbsp;";
                        }
                    }

                    if("<?=$status?>"=="downshelf"){//下架完成
                        return "&nbsp;&nbsp;<i><a onclick='redoUpshelf("+row.prt_pkid+")'>重新上架</a></i>&nbsp;&nbsp;<i><a href='javascript:void(0)' onclick='editPartno("+row.prt_pkid+")'>维护料号信息</a></i>&nbsp;&nbsp;";
                    }

                    if("<?=$status?>"=="checking"){//审核中
                        return "&nbsp;&nbsp;<i><a onclick='checkProgress("+row.l_prt_pkid+")' href='javascript:void(0)'>查看进度</a></i>&nbsp;&nbsp;";
                    }
            ...
            onSelect: function (index, row) {
                if(row){
                    $.ajax({
                        url:"<?=\yii\helpers\Url::to(['partno-ajax-info'])?>?id=" + row.prt_pkid,
                        type:"get",
    ...
    function editPartno(id){
        ...
        event && event.stopPropagation();
        $.ajax({
            url:"<?=\yii\helpers\Url::to(['partno-ajax-form'])?>?id=" + id+"&status=<?=$status?>&type=1",
            type:"get",
            success:function(res){
                $("._basic").loading("end");
...
//                $("#partno_hidden").val(row.part_no);
    ...
    //编辑商品
    function editPdt(event,id){
        event.stopPropagation();
        window.location.href="<?=\yii\helpers\Url::to(['edit-partno'])?>?id="+id+"&status=<?=$status?>";
    }
