...
<div class="create-plan content">
    <h1 class="head-first">修改仓库</h1>
    <?php $form = \yii\widgets\ActiveForm::begin([
        'id' => 'create-warehouse',
        'enableAjaxValidation' => true,
        'action' => ['/warehouse/set-warehouse/update-warehouse?id='.$id]
    ]) ?>
    <h1 class="head-second" style="text-align:left;">仓库基本信息</h1>
    <div class="space-10"></div>
    <div class="mt10-ml30">
        <label class="label-align width-70" >仓库代码：</label>
        <span class="width-180"><?=$model[0]['wh_code']?></span>
        <input type="hidden" class="wh_code value-align width-180"
               name="BsWh[wh_code]"  value="<?=$model[0]['wh_code']?>"/>
        <label class="label-align width-180"><span class="red">*</span>仓库名称：</label>
        <input  type="text" class="wh_name value-align width-188 easyui-validatebox" name="BsWh[wh_name]" data-options="required:true" maxlength="200" value="<?=$model[0]['wh_name']?>"/>
    </div>
    <div class="mt10-ml30">
...
<!--            <option value="">请选择</option>-->
            <?php foreach($downList['wh_nature'] as $key => $val){ ?>
                <option value="<?= $val['bsp_id']?>" <?=$model[0]['wh_nature']==$val['bsp_id'] ? "selected" :null ?>><?= $model[0]['wh_natures'] ?></option>
            <?php  } ?>
        </select>
        <label class="label-align width-180"><span class="red">*</span>仓库属性：</label>
        <select name="BsWh[wh_attr]" class="easyui-validatebox validateboxs value-align width-188" >
<!--            <option value="">请选择</option>-->
            <?php foreach($downList['wh_attr'] as $key => $val){ ?>
                <option value="<?= $val['bsp_id']?>" <?=$model[0]['wh_attr']==$val['bsp_id']? "selected" :null ?>><?= $model[0]['wh_attrs']?></option>
            <?php  } ?>
        </select>
    </div>
    <div class="mt10-ml30">
...
<!--            <option value="">请选择</option>-->
            <?php foreach($downList['wh_type'] as $key => $val){ ?>
                <option value="<?= $val['bsp_id']?>" <?= $model[0]['wh_type']==$val['bsp_id']? "selected" :null ?>><?= $model[0]['wh_types']?></option>
            <?php  } ?>
        </select>
...
<!--            <option value="">请选择</option>-->
            <?php foreach($downList['wh_lev'] as $key => $val){ ?>
                <option value="<?= $val['bsp_id']?>" <?=$model[0]['wh_lev']==$val['bsp_id']? "selected" :null ?>><?= $model[0]['wh_levs']?></option>
            <?php  } ?>
        </select>
    </div>
    <div class="mt10-ml30">
        ...
        <select name="BsWh[wh_yn]" class="easyui-validatebox validateboxs value-align width-180" data-options="required:true">
            <option value="N" <?=$model[0]['wh_yn']=="N"? "selected" :null ?>>否</option>
            <option value="Y" <?=$model[0]['wh_yn']=="Y"? "selected" :null ?>>是</option>
        </select>
        ...
        <select name="BsWh[yn_deliv]" class=" easyui-validatebox validateboxs value-align width-188"
            ...
            <option value="1" <?=$model[0]['yn_deliv']=="1"? "selected" :null ?>>是</option>
            <option value="0" <?=$model[0]['yn_deliv']=="0"? "selected" :null ?>>否</option>
        </select>
    </div>
    <div class="mt10-ml30">
...
<!--            <option value="" style="">请选择</option>-->
            <?php foreach($downList['people'] as $key => $val){ ?>
                <option style="width: 160px" value="<?= $val['company_name']?>"<?=$model[0]['people']==$val['company_name']? "selected" :null ?>><?= $val['company_name']?></option>
            <?php  } ?>
        </select>
    ...
    <div class="mt10-ml30">
        ...
        <div class="ax_文本框_单行_" id="u676">
            <input class="value-align text easyui-validatebox" data-options="required:'true'" id="u676_input" name="BsWh[wh_addr]" type="text" value="<?=$model[0]['wh_addr']?>" maxlength="20" onchange="this.value=this.value.substring(0, 20)" >
        </div>
    </div>
    <div class="mt10-ml30">
        ...
        <textarea class="value-align width-555" type="text" rows="3" data-options="required:true" maxlength="200" onchange="this.value=this.value.substring(0, 200)"
                  name="BsWh[remarks]"><?=$model[0]['remarks']?></textarea>
    </div>
    <div class="mt10-ml30">
        ...
        <select name="BsWh[wh_state]" class="easyui-validatebox validateboxs value-align width-180" data-options="required:true">
            <option value="Y" <?=$model[0]['wh_state']=="Y"? "selected" :null ?>>启用</option>
            <option value="N" <?=$model[0]['wh_state']=="N"? "selected" :null ?>>禁用</option>
        </select>
    </div>
    ...
    <div style="margin-left: 35px">
        <table id="User" class="table width-700">
            ...
            <tr class="height-30">
                <td><?=($key+1) ?></td>
                <td><input id="_users" name="sta[<?= $key?>][WhAdm][emp_no]" type="text"
                           data-options="required:'true'" value="<?=$val['emp_no']?>"
                           class="tabletext easyui-validatebox" ></td>
                <td><span id="_name" class="tabletext"><?=$val['staff_name']?></span></td>
                <td><input id="_phone" name="sta[<?= $key?>][WhAdm][adm_phone]"
                           data-options="required:'true',validType:'tel_mobile_c'"
                           type="text" value="<?=$val['adm_phone']?>" class="tabletext easyui-validatebox"></td>
                <td><input id="_email" name="sta[<?= $key?>][WhAdm][adm_email]"
                           class="tabletext easyui-validatebox"
                           data-option="validType:'email'"
                           type="text" value="<?=$val['adm_email']?>" class="tabletext"></td>
    ...
    <div class="mb-20 text-center" style="margin-top: 10px">
        ...
        <button class="button-white-big ml-40" type="button" id="close">取消</button>
    </div>
    <?php $form->end(); ?>
...
<script>
    ...
    function add_lites(){
        ...
        var obj=$("#user").append(
            ...
            'class="tabletext easyui-validatebox" ' +
            'data-option="validType:\'email\'">' + '</td>'+
