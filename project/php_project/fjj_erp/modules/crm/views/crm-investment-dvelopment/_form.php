...
<div class="content">
    ...
    <h2 class="head-first">
        ...
        <?php if (\Yii::$app->controller->action->id == "update") { ?>
            <span style=" color: white; font-size: 10px;">修改时间：<?php echo date("Y-m-d H:i:s"); ?> </span>
        <?php } ?>
        <?= !empty($model['cust_filernumber']) ? '<span class="head-code">档案编号:' . $model['cust_filernumber'] . '</span>' : '' ?>
    </h2>
    <h2 class="head-second">
        <a href="javascript:void(0)"><i class="icon-caret-right"></i>客户基本信息</a>
    </h2>
    <div class="ml-20 mb-10">
        <div class="mb-10 <?= isset($model['cust_id']) ? "hiden" : "" ?>">
            <label class="label-width qlabel-align"><span class="red">*</span>公司名称</label><label>：</label>
            <input class="value-width qvalue-align easyui-validatebox"
                   data-options="required:true,validType:'unique',delay:500" data-act="<?= Url::to(['validate']) ?>"
                   data-attr="cust_sname" data-id="<?= $model['cust_id']; ?>" maxlength="50" type="text"
                   name="CrmCustomerInfo[cust_sname]" value="<?= $model['cust_sname'] ?>">
            <label class="label-width qlabel-align margin"><span class="red">*</span>公司简称</label><label>：</label>
            <input class="value-width qvalue-align" maxlength="15" type="text" name="CrmCustomerInfo[cust_shortname]"
                   value="<?= $model['cust_shortname'] ?>"><br/>
        ...
        <div class="mb-10 ">
            ...
            <input class="value-width qvalue-align easyui-validatebox IsTel" type="text"
                   ...
                   value="<?= $model['cust_tel1'] ?>" maxlength="15" placeholder="请输入 xxx-xxxxxxxx">
            <label class="label-width qlabel-align margin">邮编</label><label>：</label>
            <input class="value-width qvalue-align easyui-validatebox" data-options="validType:'postcode'" type="text"
        ...
        <div class="mb-10 ">
            ...
            <input class="value-width qvalue-align easyui-validatebox add-require"
                   data-options="required:true,validType:'mobile'" type="text" name="CrmCustomerInfo[cust_tel2]"
                   value="<?= $model['cust_tel2'] ?>" placeholder="请输入 136xxxxxxxx">
            <label class="label-width qlabel-align margin"><span class="red">*</span>邮箱</label><label>：</label>
            <input class="value-width qvalue-align easyui-validatebox add-require"
                   data-options="required:'true',validType:'email'" type="text" name="CrmCustomerInfo[cust_email]"
                   value="<?= $model['cust_email'] ?>" maxlength="50" placeholder="请输入 xxx@xxx.com">
        </div>
        <?php if (isset($model['member_regweb'])) { ?>
            <div class="mb-10">
                ...
                <label class="value-width qvalue-align vertical-top add-require" id="member_regweb">
                    ...
                    <?php } ?></label>
                <label class="label-width qlabel-align margin vertical-top member_name">用户名</label><label>：</label>
                <label class="value-width qvalue-align  add-require vertical-top"
...
<!--                       value="--><?//= $model['member_regtime'] ? $model['member_regtime'] : '' ?><!--" onfocus="this.blur();">-->
    ...
    <h2 class="head-second">
        <a href="javascript:void(0)"><i class="icon-caret-right"></i>客户详细信息</a>
    </h2>
    <div class="ml-20 mb-10">
        ...
        <div class="mb-10 ">
            ...
            <input class="value-width qvalue-align Wdate" type="text" name="CrmCustomerInfo[cust_regdate]"
                   value="<?= $model['cust_regdate'] ?>" onclick="WdatePicker({skin: 'whyGreen', dateFmt: 'yyyy-MM-dd' })"  onfocus="this.blur();">
        </div>
        <div class="mb-10 ">
            ...
            <select class="value-width qvalue-align" name="CrmCustomerInfo[member_regcurr]">
                <?php foreach ($downList['tradeCurrency'] as $key => $val) { ?>
                    <option
                        value="<?= $val['bsp_id'] ?>"<?=
                    ((empty($model['member_regcurr']) ? $rmbId : $model['member_regcurr']) == $val['bsp_id']) ? "selected" : null; ?>><?= $val['bsp_svalue'] ?></option>
                <?php } ?>
            </select>
        ...
        <div class="mb-10 ">
            ...
            <input class="value-width qvalue-align easyui-validatebox" type="text"
                   name="CrmCustomerInfo[member_compwebside]" data-options="validType:'www'"
                   value="<?= $model['member_compwebside'] ?>" maxlength="50" placeholder="请输入 www.xxx.xxx">
        </div>
        <div class="mb-10 ">
            <label class="label-width qlabel-align vertical-top">范围说明</label><label class="vertical-top">：</label>
            <textarea style="width: 620px" name="CrmCustomerInfo[member_businessarea]" rows="3" maxlength="200"
                      data-options="validType:'maxLength[200]'" class="easyui-validatebox"
                      placeholder="最多输入200个字"><?= $model['member_businessarea'] ?></textarea>
        </div>
        <div class="mb-10 ">
            <label class="label-width qlabel-align vertical-top">备注</label><label class="vertical-top">：</label>
            <textarea style="width: 620px" name="CrmCustomerInfo[member_remark]" rows="3" maxlength="200"
                      class="easyui-validatebox" data-options="validType:'maxLength[200]'"
                      placeholder="最多输入200个字"><?= $model['member_remark'] ?></textarea>
        </div>
    </div>
    <h2 class="head-second">
        <a href="javascript:void(0)"><i class="icon-caret-right"></i>其他联系人信息</a>
    </h2>
            ...
            <?php foreach ($model["contacts"] as $k => $contact) { ?>
                <tr align="center">
                    ...
                    <td width="150"><input name="CrmCustomerPersion[ccper_mobile][]"
                                           class="width-150 text-center easyui-validatebox IsTel"
