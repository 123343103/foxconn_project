...
<!--        <a id="select-customer" class="ml-10" href="javascript:void(0)">客户选择</a>-->
    ...
    <div class="mb-20">
        <input id="cust_id" name="cust_id"  type="hidden" value="<?=$data['customerData']['cust_id']?>">
        <label class="width-100" for="">公司名称</label>
        <input id="cust_sname"  class="width-120"  type="text" value="<?=$data['customerData']['cust_sname'];?>" disabled>
        <label class="width-100" for="">公司简称</label>
        <input id="cust_shortname" class="width-120" type="text" value="<?=$data['customerData']['cust_shortname'];?>" disabled>
        <label class="width-100" for="">公司电话</label>
        <input  id="cust_tel1" class="width-120" type="text" value="<?=$data['customerData']['cust_tel1'];?>" disabled>
    </div>
    ...
    <div class="mb-20">
        ...
        <input id="cust_contacts"  class="width-120"  type="text" value="<?=$data['customerData']['cust_contacts'];?>" disabled>
        <label class="width-100" for="">职位</label>
        <input id="cust_position"  class="width-120" type="text" value="<?=$data['customerData']['cust_position'];?>" disabled>
        <label class="width-100" for="">手机号码</label>
        <input id="cust_tel2"  class="width-120" type="text" value="<?=$data['customerData']['cust_tel2'];?>" disabled>
        <label class="width-100" for="">邮箱</label>
        <input id="cust_email"  class="width-120"  type="text" value="<?=$data['customerData']['cust_email'];?>" disabled>
    </div>
    ...
    <div class="mb-20">
        ...
        <select class="width-120" id="cust_ismember" disabled>
            <option <?=$data['customerData']['cust_ismember']==1?"selected":""?> value="1">是</option>
            <option <?=$data['customerData']['cust_ismember']==0?"selected":""?> value="0">否</option>
        </select>
        <label class="width-100" for="">注册时间</label>
        <input id="cust_regdate" class="width-120 select-date" type="text" value="<?=$data['customerData']['cust_regdate'];?>" disabled>
        ...
        <select class="width-120" id="member_type" disabled>
            ...
            <?php foreach($downList[member_type] as $k=>$v){ ?>
                <option <?=$v[bsp_id]==$data[customerData][member_type]?"selected":""?> value="<?=$v[bsp_id];?>"><?=$v[bsp_svalue];?></option>
            <?php } ?>
        </select>
        <label class="width-100" for="">会员名</label>
        <input class="width-120" type="text" value="<?=$data['customerData']['member_name'];?>" disabled>
    </div>
    ...
    <div class="mb-20">
        ...
        <select class="width-120" id="" disabled>
            ...
            <?php foreach($downList[cust_businesstype] as $k=>$v){ ?>
                <option <?=$v[bsp_id]==$data['customerData']['cust_businesstype']?"selected":""?> value="<?=$v[bsp_id];?>"><?=$v[bsp_svalue];?></option>
            <?php } ?>
        </select>
        <label class="width-100" for="">经营范围</label>
        <input class="width-120" type="text" name="member_businessarea" value="<?=$data['customerData'][member_businessarea];?>"  disabled>
        <label class="width-100" for="">需求类别</label>
        <input class="width-120" type="text" value="<?=$data['customerData']['member_reqdesription'];?>" disabled>
    </div>
    ...
    <div class="mb-20">
        ...
        <select class="width-120" id="" disabled>
            ...
            <?php foreach($downList[productType] as $k=>$v){ ?>
                <option <?=$v[category_id]==$data['customerData'][member_reqitemclass]?"selected":""?> value="<?=$v[category_id];?>"><?=$v[category_sname];?></option>
            <?php } ?>
        </select>
        ...
        <select class="width-120" id="" disabled>
            ...
            <?php foreach($downList[member_reqflag] as $k=>$v){ ?>
                <option <?=$v['bsp_id']==$data['customerData']['member_reqflag']?"selected":""?> value="<?=$v[bsp_id];?>"><?=$v[bsp_svalue];?></option>
            <?php } ?>
        </select>
        <label class="width-100" for="">需求特征</label>
        <input id="member_reqcharacter" class="width-120" type="text" value="<?=$data['customerData']['member_reqcharacter'];?>" disabled>
    </div>
    ...
    <div class="mb-20">
        <input id="visit_cust_id" type="hidden" name="CrmVisitRecord[cust_id]" value="<?=$data[customerData][cust_id]?>">
        <input id="visit_cust_name" type="hidden" name="CrmVisitRecord[cust_name]" value="<?=$data[customerData][cust_sname]?>">
        <input type="hidden" name="CrmVisitRecordChild[sil_staff_code]" value="<?= $visitPerson->staff_code ?>">
        <label class="width-100" for="">拜访人</label>
        <input name="CrmVisitRecordChild[sil_staff_code]" type="text" class="width-150" value="<?= $visitPerson->staff_name ?>">
        <label class="width-100" for="">拜访日期</label>
        <input type="text" name="CrmVisitRecordChild[sil_date]" class="width-150 select-date" value="<?= !empty($data['recordChildData']) ? $data['recordChildData']['sil_date'] : null ?>">
        <label class="width-100" for="">拜访类型</label>
        <select name="CrmVisitRecordChild[sil_type]" id="" class="width-150">
            <option value="">选择</option>
            <?php foreach($downList['visit_type'] as $v){?>
                <option <?=$data['recordChildData']['sil_type']==$v[bsp_id]?"selected":""?> value="<?=$v[bsp_id];?>"><?=$v[bsp_svalue];?></option>
            <?php } ?>
        </select>
    ...
    <div class="mb-20">
        ...
        <input id="startDate" data-target="#endDate" data-type="le" data-options="required:true,validType:'timeCompare'" name="arriveDate" type="text" class="width-150 easyui-validatebox select-date-time" value="<?=$data['recordChildData']['start']?>">
        <label class="width-100" for="">结束时间</label>
        <input id="endDate" data-target="#startDate" data-type="ge" data-options="required:true,validType:'timeCompare'" type="text" name="leaveDate" class="width-150 easyui-validatebox select-date-time" value="<?=$data['recordChildData']['end']?>">
        <label class="width-100" for="">拜访用时</label>
        <input type="text" class="width-30 text-center day" id="day-1" name="day-1" value="<?=$sil_time[0];?>"/> 天
        <input type="text" class="width-30 text-center hours" id="hours-1" name="hours-1" value="<?=$sil_time[1];?>"/> 时
        <input type="text" class="width-30 text-center minutes" id="minutes-1" name="minutes-1" value="<?=$sil_time[2];?>"/> 分
    </div>
    <div class="mb-20">
        <label class="width-100" for="">拜访內容</label>
        <textarea style="vertical-align: top;" name="CrmVisitRecordChild[execute_project]" id="" cols="80" rows="5"><?=$data['recordChildData']['execute_project']?></textarea>
    </div>
    <div class="mb-20">
        <label style="vertical-align: top;" class="width-100" for="">客户反馈或<br/>拜访总结</label>
        <textarea name="CrmVisitRecordChild[sil_interview_conclus]" id="" cols="80" rows="5"><?=$data['recordChildData']['sil_interview_conclus']?></textarea>
    </div>
    <div class="mb-20">
        <label style="vertical-align: top;" class="width-100" for="">备注</label>
        <textarea name="CrmVisitRecordChild[remark]" id="" cols="80" rows="5"><?=$data['recordChildData']['remark']?></textarea>
    </div>
...
<script>
    $(function(){
        var customer;
        ajaxSubmitForm($("#visit-edit-form"));
