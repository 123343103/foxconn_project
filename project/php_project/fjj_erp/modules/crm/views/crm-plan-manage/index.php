...
/* @var $searchModel app\modules\ptdt\models\search\CrmVisitPlanSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = '行程日历';
$this->params['homeLike'] = ['label' => '客户关系管理'];
$this->params['breadcrumbs'][] = ['label' => '行程日历'];
?>
<style>
    .fancybox-inner{
        width:700px !important;
    }
    .fc-toolbar .fc-state-active ,.fc-toolbar button:focus {
        z-index: 0;
    }
...
//                });
            ...
            loading:function(isLoading,view){
                if( !isLoading){
                    $(".fc-title").each(function(){
                        $(this).attr("title",$(this).html().decode());
                    })
                }
            },
            ...
            select: function(start, end, jsEvent, view){
                var startDate =moment(start).format("YYYY-MM-DD")+"@08:00";
                var endDate = moment(start).format("YYYY-MM-DD")+"@18:00";
...
//                //调用fancybox弹出层
            ...
            eventClick:  function(Event, jsEvent, view) {
                ...
                var hid=Event.sih_id;
                var type=Event.type;
//                return false;
                var url='';
                if(type == 10) {
                    url = "<?= Url::to(['/crm/crm-visit-plan/view'])?>?id=" + id;
                }else if(type == 20 || type == 30){
                    url= "<?= Url::to(['/crm/crm-visit-record/view-record'])?>?childId="+cid
//                }if(type == 40){
//                    url= "<?//= Url::to(['/crm/crm-return-visit/view'])?>//?id="+hid+"&childId="+cid
                }
