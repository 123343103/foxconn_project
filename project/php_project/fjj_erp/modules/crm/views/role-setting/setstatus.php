...
<div class="no-padding " style="width: 500px">
    ...
    <div class="mb-10">
        <label class="label-width qlabel-align " style="margin-left: 20px;width: 60px">销售角色<label>：</label></label>
        <label class="text-top name" style="text-align: left;width: 439px;line-height: 18px;display: inline"><?=$saleinfo?></label>

        </div>
    <div class="mb-10">
        <label class="label-width qlabel-align " style="margin-left: 20px;width: 60px">状态<label>：</label></label>
        <select  type="text" name="sarole_status" style="width: 150px" class="sarole_status">
...
<script>
    $(function () {
        $("#submit").on('click',function () {
            $.ajax({
                type: "POST",
                data: {id: "<?=$idarr?>",sarole_status:$(".sarole_status").val()},
                dataType: "json",
