...
$this->params['breadcrumbs'][] = ['label' => '客户资料查询'];
?>
<div class="content">
...
<script>
    $(function () {
        $("#data").datagrid({
            ...
            checkOnSelect: true,
            columns: [[
                {field: "cust_filernumber",title:"系统编号",width:120},
                ...
                {field: "staff_email",title:"邮箱",width:100},
                {field: "custManager",title:"客户经理人",width:100},
                {field: "district_name",title:"所在地区",width:100},
                {field: "cust_salearea",title:"所在军区",width:100},
                ...
                {field: "cust_industrytype",title:"行业类型",width:100},
                ...
                {field: "cust_islisted",title:"是否上市",width:80,formatter:function(value,row,index){
                ...
                {field: "cust_ismember",title:"是否会员",width:80,formatter:function(value,row,index){
                ...
                {field: "bsp_svalue",title:"会员类型",width:100},
                {field: "YN",title:"认证状态",width:100,formatter:function(value,row,index){
                    if (row.cust_ismember==0) {
                        return  '无效';
                    } else{
                        return '有效';
...
//            });
            ...
            layer.confirm("确定导出信息?",
                ...
                function () {
                    if (window.location.href = " <?=Url::to(['index', 'export' => '1', 'CrmCustomerInfoSearch[cust_type]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['cust_type'] : null, 'CrmCustomerInfoSearch[property]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['property'] : null, 'CrmCustomerInfoSearch[cust_ismember]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['cust_ismember'] : null, 'CrmCustomerInfoSearch[cust_salearea]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['cust_salearea'] : null,'CrmCustomerInfoSearch[cust_area]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['cust_area'] : null,'CrmCustomerInfoSearch[cust_sname]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['cust_sname'] : null,'CrmCustomerInfoSearch[custManager]' => !empty($queryParam) ? $queryParam['CrmCustomerInfoSearch']['custManager'] : null])?>") {
                        layer.closeAll();
                    } else {
                        layer.alert('导出客户资料错误!', {icon: 0})
                    }
                },
                function () {
                    layer.closeAll();
